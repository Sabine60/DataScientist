...
def table_length(handles)
puts "Il y a #{handles.length} handles dans cette array"
end

    #EXO 2

def handle_small_length(handles)
    point = handles[0]
    handles.each do |handle|
        if handle.length < point.length
            point = handle
        end
    end
    puts point
...
def capitalFirstLetter(handles)
    count = 0 
    handles.each do |handle| 
        if is_upcase?(handle[1]) 
            handle[1]
            count += 1
        end
    end
...
def alphabetical_sort(handles)
    puts handles.sort
end

    #EXO 6

def length_sort(handles)
